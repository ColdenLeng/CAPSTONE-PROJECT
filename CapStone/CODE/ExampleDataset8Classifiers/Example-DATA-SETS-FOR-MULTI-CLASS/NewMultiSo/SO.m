...
%                                                                   %
%___________________________________________________________________%
function [sFeat,Sf,Nf,Stp_runs,Xfood, fval,gbest_t] = SO(feat,label,N,T,lb,ub,dim,fobj,Stp_crit,HO)
%function [Stp_runs,Xfood, fval,gbest_t] = SO(N,ans,T,Stp_crit,lb,ub,dim,fobj)
...
for i=1:N
 ...
 %
 %fitness(i) = feval (fobj,feat, label, (X(i,:)>0.5), HO);
 fitness(i) = fobj(feat, label, (X(i,:)>0.5), HO);
 %
end
...
if Q<Threshold
    ...
    for j=1:Nm
        ...
        %y = fobj(feat, label, Xnewm(j,:)>0.5, HO);
        y = feval(fobj,feat,label, Xnewm(j,:)>0.5, HO);
    ...
    for j=1:Nf
        ...
        y = feval(fobj, feat, label, Xnewm(i,:)>0.5, HO);
        %y = fobj(feat, label, Xnewm(i,:)>0.5, HO);
        if y<fitness_f(j)
            fitness_f(j)=y;
            Xf(j,:)= Xnewf(j,:);
        end
    ...
    Stp_runs=t*N;
    if t == 1
        fprintf('\nIteration %d Best = %f', t, gbest_t(t));
    end

    if t > 1
        if gbest_t(t) ~= gbest_t(t-1)
            fprintf('\nIteration %d Best = %f', t,gbest_t(t));
        end
    end
